...
def upgrade() -> None:
    ...
    op.add_column('users', sa.Column('fs_uniquifier', sa.String(length=64), nullable=False, server_default=text("(gen_random_uuid())")))

    user_ids = op.get_bind().execute("SELECT id FROM users")

    for user_id in user_ids:
        op.execute(f"UPDATE users SET fs_uniquifier = '{uuid.uuid4().hex}' WHERE id = {user_id[0]}")
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(["user_id"], ["users.fs_uniquifier"], name='fk_oauth2client_user_id', ondelete="CASCADE"),
        sa.PrimaryKeyConstraint("id")
    )

    op.create_index(op.f("ix_oauth2client_client_id"), "oauth2client", ["client_id"], unique=False)

    op.execute("""INSERT INTO oauth2client (client_id, client_secret, client_id_issued_at, client_secret_expires_at, user_id, ratelimit_multiplier, last_activity, client_metadata)
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(["client_id"], ["oauth2client.id"], name='fk_oauth2token_client_id', ondelete="CASCADE"),
        sa.ForeignKeyConstraint(["user_id"], ["users.fs_uniquifier"], name='fk_oauth2token_user_id', ondelete="CASCADE"),
        sa.PrimaryKeyConstraint("id"),
    ...
    op.execute("""INSERT INTO oauth2token (token_type, access_token, refresh_token, scope, issued_at, access_token_revoked_at, refresh_token_revoked_at, expires_in, user_id, client_id, is_personal, is_internal)
...
def downgrade() -> None:
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(['user_id'], ['users.id'], name='oauth2client_user_id_fkey', ondelete='CASCADE'),
        sa.PrimaryKeyConstraint('client_id')
    )

    op.execute("""INSERT INTO oauth2client_old (client_id, client_secret, user_id, ratelimit, last_activity, name, description, website, is_confidential, _default_scopes, _redirect_uris, created, is_internal)
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(['user_id'], ['users.id'], name='oauth2token_user_id_fkey', ondelete='CASCADE'),
        sa.ForeignKeyConstraint(['client_id'], ['oauth2client_old.client_id'], name='oauth2token_client_id_fkey', ondelete='CASCADE'),
        sa.PrimaryKeyConstraint('id'),
        sa.UniqueConstraint('access_token'),
        sa.UniqueConstraint('refresh_token')
    )

    op.execute("""INSERT INTO oauth2token_old (token_type, access_token, refresh_token, _scopes, expires, user_id, client_id, is_personal, is_internal)
    ...
    op.create_unique_constraint('oauth2token_refresh_token_key', 'oauth2token_old', ['refresh_token'])
    # <<< oauth2token
